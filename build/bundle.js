(function () {
	...
	/* layercake/SVG.html generated by Svelte v2.16.0 */
	...
	function add_css() {
		...
		style.id = 'svelte-1rpwb0m-style';
		style.textContent = ".layercake-svg.svelte-1rpwb0m{position:absolute;top:0;left:0;width:100%;height:100%}";
	...
	function create_main_fragment(component, ctx) {
		var svg, g, g_transform_value, svg_class_value, svg_viewBox_value;
		...
		return {
			...
			l(nodes) {
				svg = claimElement(nodes, "svg", { class: true, width: true, height: true, viewBox: true, preserveAspectRatio: true }, true);
			...
			h() {
				...
				setAttribute(svg, "class", svg_class_value = "layercake-svg layercake-svg-" + ctx.layoutIndex + " svelte-1rpwb0m");
			...
			p(changed, ctx) {
				if (changed.layers || changed.props) {
				...
				if ((changed.layoutIndex) && svg_class_value !== (svg_class_value = "layercake-svg layercake-svg-" + ctx.layoutIndex + " svelte-1rpwb0m")) {
	...
	function create_each_block(component, ctx) {
		...
		return {
			...
			p(changed, ctx) {
				var switch_instance_changes = (changed.layers || changed.props) ? getSpreadUpdate(switch_instance_spread_levels, [
					(changed.layers) && ctx.layer.options,
	...
	function SVG(options) {
		...
		if (!document.getElementById("svelte-1rpwb0m-style")) add_css();
	...
	/* layercake/LayerCake.html generated by Svelte v2.16.0 */



	function props$1(state) {
		return state;
	}

	function width({ defaultWidth, containerWidth }) {
	  return containerWidth || defaultWidth;
	}
	function height({ defaultHeight, containerHeight }) {
	  return containerHeight || defaultHeight;
	}
	function innerWidth({ padding, width }) {
	  return width - padding.left - padding.right;
	}
	function innerHeight({ padding, height }) {
	  return height - padding.top - padding.bottom;
	}
	function xScaled({ xGet, xScale }) {
	  return d => xScale(xGet(d));
	}
	function yScaled({ yGet, yScale }) {
	...
	function xScale({ xMin, xMax, innerWidth }) {
	  return linear$2()
	    .domain([xMin, xMax])
	    .range([0, innerWidth]);
	}
	function yScale({ yMin, yMax, innerHeight }) {
	  return linear$2()
	    .domain([yMin, yMax])
	    .range([innerHeight, 0]);
	}
	function xMin({ data, xGet }) {
	  if (!data || !xGet) {
	    return;
	  }
	...
	function add_css$1() {
		var style = createElement("style");
		style.id = 'svelte-5d5a20-style';
		style.textContent = ".layercake,.layercake *{box-sizing:border-box}.layercake{width:100%;height:100%}";
		append(document.head, style);
	}
	...
	function create_main_fragment$1(component, ctx) {
		...
		var if_block = (ctx.svgLayers && ctx.svgLayers.length) && create_if_block(component, ctx);

		function div_resize_handler() {
			component.set({ containerWidth: div.clientWidth, containerHeight: div.clientHeight });
		}
		...
		return {
			...
			p(changed, ctx) {
				if (ctx.svgLayers && ctx.svgLayers.length) {
					if (if_block) {
	...
	// (5:1) {#if svgLayers && svgLayers.length}
	function create_if_block(component, ctx) {

		var svg_spread_levels = [
			ctx.props,
			{ layers: ctx.svgLayers }
		];

		var svg_initial_data = {};
		for (var i = 0; i < svg_spread_levels.length; i += 1) {
			svg_initial_data = assign(svg_initial_data, svg_spread_levels[i]);
		}
		var svg = new SVG({
			root: component.root,
			store: component.store,
		...
		return {
			...
			p(changed, ctx) {
				var svg_changes = (changed.props || changed.svgLayers) ? getSpreadUpdate(svg_spread_levels, [
					(changed.props) && ctx.props,
					(changed.svgLayers) && { layers: ctx.svgLayers }
				]) : {};
	...
	function LayerCake(options) {
		init(this, options);
		this._state = assign(data$1(), options.data);

		this._recompute({ defaultWidth: 1, containerWidth: 1, defaultHeight: 1, containerHeight: 1, padding: 1, width: 1, height: 1, x: 1, data: 1, xGet: 1, xMin: 1, xMax: 1, innerWidth: 1, xScale: 1, y: 1, yGet: 1, yMin: 1, yMax: 1, innerHeight: 1, yScale: 1 }, this._state);
		this._intro = true;

		if (!document.getElementById("svelte-5d5a20-style")) add_css$1();
	...
	assign(LayerCake.prototype, proto);

	LayerCake.prototype._recompute = function _recompute(changed, state) {
		if (changed.defaultWidth || changed.containerWidth) {
			if (this._differs(state.width, (state.width = width(state)))) changed.width = true;
		}

		if (changed.defaultHeight || changed.containerHeight) {
		...
		if (changed.x) {
			if (this._differs(state.xGet, (state.xGet = xGet(state)))) changed.xGet = true;
		}

		if (changed.data || changed.xGet) {
			if (this._differs(state.xMin, (state.xMin = xMin(state)))) changed.xMin = true;
			if (this._differs(state.xMax, (state.xMax = xMax(state)))) changed.xMax = true;
		}

		if (changed.xMin || changed.xMax || changed.innerWidth) {
			if (this._differs(state.xScale, (state.xScale = xScale(state)))) changed.xScale = true;
		}

		if (changed.xGet || changed.xScale) {
			if (this._differs(state.xScaled, (state.xScaled = xScaled(state)))) changed.xScaled = true;
		}

		if (changed.y) {
			if (this._differs(state.yGet, (state.yGet = yGet(state)))) changed.yGet = true;
		}

		if (changed.data || changed.yGet) {
			if (this._differs(state.yMin, (state.yMin = yMin(state)))) changed.yMin = true;
			if (this._differs(state.yMax, (state.yMax = yMax(state)))) changed.yMax = true;
		}

		if (changed.yMin || changed.yMax || changed.innerHeight) {
			if (this._differs(state.yScale, (state.yScale = yScale(state)))) changed.yScale = true;
		}

		if (changed.yGet || changed.yScale) {
			if (this._differs(state.yScaled, (state.yScaled = yScaled(state)))) changed.yScaled = true;
		}
	...
	function create_main_fragment$2(component, ctx) {
		...
		return {
			...
			h() {
				...
				setAttribute(rect, "width", ctx.innerWidth);
				setAttribute(rect, "height", ctx.innerHeight);
	...
	function ChartOutline(options) {
		...
		if (!document.getElementById("svelte-p1gazz-style")) add_css$2();
	...
	/* components/ChartLine.html generated by Svelte v2.16.0 */

	function path({ data, xScaled, yScaled }) {
	  if (!data || !xScaled || !yScaled) {
	    return "";
	  }
	...
	function add_css$3() {
		var style = createElement("style");
		style.id = 'svelte-11gj2p0-style';
		style.textContent = ".chart-line.svelte-11gj2p0{fill:none;stroke:red;stroke-linejoin:round;stroke-linecap:round;stroke-width:2}";
		append(document.head, style);
	}
	...
	function create_main_fragment$3(component, ctx) {
		...
		return {
			...
			l(nodes) {
				path_1 = claimElement(nodes, "path", { class: true, d: true }, true);
				var path_1_nodes = children(path_1);

				path_1_nodes.forEach(detachNode);
				this.h();
			},

			h() {
				setAttribute(path_1, "class", "chart-line svelte-11gj2p0");
	...
	function ChartLine(options) {
		...
		this._recompute({ data: 1, xScaled: 1, yScaled: 1 }, this._state);
		this._intro = true;

		if (!document.getElementById("svelte-11gj2p0-style")) add_css$3();
	...
	assign(ChartLine.prototype, proto);

	ChartLine.prototype._recompute = function _recompute(changed, state) {
		if (changed.data || changed.xScaled || changed.yScaled) {
	...
	function add_css$4() {
		var style = createElement("style");
		style.id = 'svelte-83e0ko-style';
		style.textContent = ".chart.svelte-83e0ko{position:relative;box-sizing:border-box;height:20em;border:1px solid #bbccdd}";
		append(document.head, style);
	}
	...
	function create_main_fragment$4(component, ctx) {
		...
		var layercake_spread_levels = [
			ctx.props
		];

		var layercake_initial_data = {};
		for (var i = 0; i < layercake_spread_levels.length; i += 1) {
			layercake_initial_data = assign(layercake_initial_data, layercake_spread_levels[i]);
		}
		var layercake = new LayerCake({
			root: component.root,
			store: component.store,
			data: layercake_initial_data
		});

		return {
			c() {
				div = createElement("div");
				layercake._fragment.c();
				this.h();
			},
			...
			l(nodes) {
				...
				layercake._fragment.l(div_nodes);
				div_nodes.forEach(detachNode);
				this.h();
			},

			h() {
				div.className = "chart svelte-83e0ko";
			},

			m(target, anchor) {
				insert(target, div, anchor);
				layercake._mount(div, null);
			},

			p(changed, ctx) {
				var layercake_changes = changed.props ? getSpreadUpdate(layercake_spread_levels, [
					ctx.props
				]) : {};
				layercake._set(layercake_changes);
			},
	...
	function Chart(options) {
		...
		if (!document.getElementById("svelte-83e0ko-style")) add_css$4();
	...
	Chart.prototype._recompute = function _recompute(changed, state) {
		if (this._differs(state.props, (state.props = props$2(exclude(state, "props"))))) changed.props = true;
	};
	...
	function create_main_fragment$5(component, ctx) {
		...
		var chart_initial_data = { data: ctx.exampleData };
		var chart = new Chart({
			root: component.root,
			store: component.store,
		...
		return {
			...
			p(changed, ctx) {
				...
				var chart_changes = {};
				if (changed.exampleData) chart_changes.data = ctx.exampleData;
				chart._set(chart_changes);
			},
	...
	function Page(options) {
		...
		if (!document.getElementById("svelte-py4awp-style")) add_css$5();
